...
def evaluate_model(model, splits, threshold=0.5, include_train = False):
    ...
    for X, y, name in zip(Xs, ys, names):
        ...
        ax.plot([fpr1,fpr1],[-.05,tpr1],'--', color='black')
        ax.plot([-.05,fpr1],[tpr1,tpr1],'--', color='black')
        ...
        plot_precision_recall_curve(model, X, y, ax=ax2)
        ...
        df3 = pd.DataFrame({'precision':precision,'recall':recall,'thresholds':thresholds2})
        ...
        ax2.plot([recall1,recall1],[min_precision-.05,precision1], '--', color='black')
        ax2.plot([min_recall-.05,recall1],[precision1,precision1], '--', color='black')
